public class BST<T extends Comparable<T>> 
{
	...
	public void deleteTree(BST<T> tree)
	{
		if (tree != null)
		{
			...
			{
				tree.replaceMeWith(tree.getRight());
			}
			else if (tree.getLeft() != null && tree.getRight() == null)
			{
				tree.replaceMeWith(tree.getLeft());
			}
			else if (tree.getLeft() != null && tree.getRight() != null)
			{
	...
	private void removeChild(BST<T> child)
	{
		...
		{
			throw new IllegalStateException(String.format("Cannot find child %s%n", child));
		}
	}

	private void replaceMeWith(BST<T> tree)
	{
		if (getParent().getLeft() == this)
			getParent().setLeft(tree);
		else if (getParent().getRight() == this)
			getParent().setRight(tree);
		tree.setParent(getParent());
	}
