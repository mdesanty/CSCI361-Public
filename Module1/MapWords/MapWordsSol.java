...
public class MapWordsSol
{
  ...
  {
    MapWordsSol main = new MapWordsSol();
    main.mapWords();
...
// ***********************************************
  ...
  {
    for (String word : m_map.keySet())
    {
        int count = m_map.get(word);
        if (!m_map2.containsKey(count))
          m_map2.put(count, new ArrayList<String>());
        List<String> words = m_map2.get(count);
        words.add(word);
    }
  }
  ...
  {
    for (int count : m_map2.keySet())
    {
      if (count >= MIN_COUNT)
      {
        List<String> words = m_map2.get(count);
        System.out.printf("%s -> %s%n", count, words);
      }
    }
  }
}
